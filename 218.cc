...
double perimeter(Point hull[],int i, int j){ //Distance from i to j in Euclidean distance
  return sqrt((hull[i].x-hull[j].x)*(hull[i].x-hull[j].x)+(hull[i].y-hull[j].y)*(hull[i].y-hull[j].y));
}
...
int main(void)
{
  ...
  while (cin >> n && n > 0) {
    ...
    */
    ...
    cout << endl;
    double distance = perimeter(hull,0,hull_size-1);
    for(i = 1; i < hull_size; ++i){
      distance += perimeter(hull,i,i-1);
    }
    cout<<"Perimeter length = "<<fixed<<setprecision(2)<<distance<<endl;
    delete[] polygon;
    delete[] hull;
    cases++;
  }
